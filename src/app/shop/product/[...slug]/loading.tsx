        ...
        <div className="flex flex-col-reverse lg:flex-row lg:space-x-3.5">
          <div className="flex lg:flex-col space-x-3 lg:space-x-0 lg:space-y-3.5 w-full lg:w-fit">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-gray-200 rounded-[13px] w-full max-w-[111px] aspect-square" />
            ))}
          </div>
          <div className="bg-gray-200 rounded-[20px] w-full min-h-[330px] lg:min-h-[530px] mb-3 lg:mb-0" />
        </div>
        <div className="space-y-4">
          ...
          <div className="flex gap-3 flex-wrap">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-10 bg-gray-200 rounded-full w-20" />
            ))}
          </div>
